...
class SDNATagQueryWidget : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SDNATagQueryWidget )
	...
	struct FEditableDNATagQueryDatum
	{
		/** Constructor */
		FEditableDNATagQueryDatum(class UObject* InOwnerObj, struct FDNATagQuery* InTagQuery, FString* InTagExportText=nullptr)
		...
		/** Tag query to edit */
		struct FDNATagQuery* TagQuery; 

		/** The export text for FDNATagQuery, useful in some circumstances */
		FString* TagQueryExportText;
	};

	/** Construct the actual widget */
	void Construct(const FArguments& InArgs, const TArray<FEditableDNATagQueryDatum>& EditableTagQueries);
...
private:
	...
	TArray<FEditableDNATagQueryDatum> TagQueries;
	...
	class UEditableDNATagQuery* CreateEditableQuery(FDNATagQuery& Q);
	TWeakObjectPtr<class UEditableDNATagQuery> EditableQuery;

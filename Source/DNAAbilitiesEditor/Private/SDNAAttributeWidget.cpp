...
void SAttributeListWidget::Construct(const FArguments& InArgs)
{
	struct Local
	{
		static void AttributeToStringArray(const UProperty& Property, OUT TArray< FString >& StringArray)
		{
			...
			if ((Class->IsChildOf(UDNAAttributeSet::StaticClass()) && !Class->ClassGeneratedBy) ||
				(Class->IsChildOf(UDNAAbilitySystemComponent::StaticClass()) && !Class->ClassGeneratedBy))
			{
...
TSharedPtr<FAttributeViewerNode> SAttributeListWidget::UpdatePropertyOptions()
{
	...
	for (TObjectIterator<UClass> ClassIt; ClassIt; ++ClassIt)
	{
		...
		// UDNAAbilitySystemComponent can add 'system' attributes
		if (Class->IsChildOf(UDNAAbilitySystemComponent::StaticClass()) && !Class->ClassGeneratedBy)
		{
			for (TFieldIterator<UProperty> PropertyIt(Class, EFieldIteratorFlags::ExcludeSuper); PropertyIt; ++PropertyIt)
			{
				UProperty* Property = *PropertyIt;
				...
				if (Property->HasMetaData(TEXT("SystemDNAAttribute")) == false)
				{
					continue;
				}
...
void SDNAAttributeWidget::Construct(const FArguments& InArgs)
{
	FilterMetaData = InArgs._FilterMetaData;
	...
	SAssignNew(ComboButton, SComboButton)
		.OnGetMenuContent(this, &SDNAAttributeWidget::GenerateAttributePicker)
		.ContentPadding(FMargin(2.0f, 2.0f))
		.ToolTipText(this, &SDNAAttributeWidget::GetSelectedValueAsString)
		.ButtonContent()
		[
			SNew(STextBlock)
			.Text(this, &SDNAAttributeWidget::GetSelectedValueAsString)
		];
...
void SDNAAttributeWidget::OnAttributePicked(UProperty* InProperty)
{
...
TSharedRef<SWidget> SDNAAttributeWidget::GenerateAttributePicker()
{
	FOnAttributePicked OnPicked(FOnAttributePicked::CreateRaw(this, &SDNAAttributeWidget::OnAttributePicked));
...
FText SDNAAttributeWidget::GetSelectedValueAsString() const
{
	if (SelectedProperty)
	{

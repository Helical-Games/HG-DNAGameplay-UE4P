...
{
	MyProperty = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FDNAAttribute,Attribute));
	OwnerProperty = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FDNAAttribute,AttributeOwner));
	NameProperty = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FDNAAttribute,AttributeName));
...
{
	MyProperty = DetailLayout.GetProperty("PropertyReference", UDNAAttributeSet::StaticClass());

	PropertyOptions.Empty();
	PropertyOptions.Add(MakeShareable(new FString("None")));

	for( TFieldIterator<UProperty> It(UDNAAttributeSet::StaticClass(), EFieldIteratorFlags::ExcludeSuper) ; It ; ++It )
	{
...
UCurveTable * FScalableFloatDetails::GetCurveTable()
{
	...
	if (CurveTable == NULL)
	{
		CurveTable = IDNAAbilitiesModule::Get().GetDNAAbilitySystemGlobals()->GetGlobalCurveTable();
	}

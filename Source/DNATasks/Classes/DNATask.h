...
#include "DNATask.generated.h"

class AActor;
class UDNATaskResource;
class UDNATasksComponent;

DNATASKS_API DECLARE_LOG_CATEGORY_EXTERN(LogDNATasks, Log, All);

UENUM()
enum class EDNATaskState : uint8
{
	Uninitialized,
	AwaitingActivation,
	Paused,
...
UENUM()
enum class EDNATaskResourceOverlapPolicy : uint8
{
...
struct DNATASKS_API FDNAResourceSet
{
...
public:
	...
	explicit FDNAResourceSet(FFlagContainer InFlags = 0) : Flags(InFlags)
	{}
	...
	FDNAResourceSet& AddID(uint8 ResourceID)
	{
		ensure(ResourceID < MaxResources);
		Flags |= (1 << ResourceID);
		return *this;
	}
	FDNAResourceSet& RemoveID(uint8 ResourceID)
	{
		ensure(ResourceID < MaxResources);
		Flags &= ~(1 << ResourceID);
		return *this;
	...
	FDNAResourceSet& AddSet(FDNAResourceSet Other)
	{
		Flags |= Other.Flags;
		return *this;
	}
	FDNAResourceSet& RemoveSet(FDNAResourceSet Other)
	{
		Flags &= ~Other.Flags;
		return *this;
	}
	...
	bool HasAllIDs(FDNAResourceSet Other) const
	{
		return (Flags & Other.Flags) == Flags;
	}
	bool HasAnyID(FDNAResourceSet Other) const
	{
		return (Flags & Other.Flags) != 0;
	}
	FDNAResourceSet GetOverlap(FDNAResourceSet Other) const
	{
		return FDNAResourceSet(Flags & Other.Flags);
	}
	FDNAResourceSet GetDifference(FDNAResourceSet Other) const
	{
		return FDNAResourceSet(Flags & ~(Flags & Other.Flags));
	}

	bool operator==(const FDNAResourceSet& Other) const
	{
	...
	static FDNAResourceSet AllResources()
	{
		return FDNAResourceSet(FFlagContainer(-1));
	}

	static FDNAResourceSet NoResources()
	{
		return FDNAResourceSet(FFlagContainer(0));
	}
...
class DNATASKS_API UDNATask : public UObject, public IDNATaskOwnerInterface
{
	GENERATED_BODY()

public:

	DECLARE_DYNAMIC_MULTICAST_DELEGATE(FGenericDNATaskDelegate);

	UDNATask(const FObjectInitializer& ObjectInitializer);
	
	/** Called to trigger the actual task once the delegates have been set up */
	UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true"), Category = "DNA Tasks")
...
protected:
	...
	void InitTask(IDNATaskOwnerInterface& InTaskOwner, uint8 InPriority);

public:
	virtual void InitSimulatedTask(UDNATasksComponent& InDNATasksComponent);
	...
	template <class T>
	FORCEINLINE static T* NewTask(TScriptInterface<IDNATaskOwnerInterface> TaskOwner, FName InstanceName = FName())
	{
	...
	template <class T>
	inline static T* NewTask(IDNATaskOwnerInterface& TaskOwner, FName InstanceName = FName());
	...
	UFUNCTION(BlueprintCallable, Category="DNATasks")
	void EndTask();
	...
	FORCEINLINE FDNAResourceSet GetRequiredResources() const { return RequiredResources; }
	FORCEINLINE FDNAResourceSet GetClaimedResources() const { return ClaimedResources; }
	
	FORCEINLINE EDNATaskState GetState() const { return TaskState; }
	FORCEINLINE bool IsActive() const { return (TaskState == EDNATaskState::Active); }
	FORCEINLINE bool IsPaused() const { return (TaskState == EDNATaskState::Paused); }
	FORCEINLINE bool IsFinished() const { return (TaskState == EDNATaskState::Finished); }
	
	UDNATask* GetChildTask() const { return ChildTask; }
	IDNATaskOwnerInterface* GetTaskOwner() const { return TaskOwner.IsValid() ? &(*TaskOwner) : nullptr; }
	UDNATasksComponent* GetDNATasksComponent() { return TasksComponent.Get(); }
	UDNATasksComponent* GetDNATasksComponent() const { return TasksComponent.Get(); }
	 ...
	 *
	 *	@note: Calling this function makes sense only until the task is being passed over to the DNATasksComponent.
	 *	Once that's that resources data is consumed and further changes won't get applied 
	 */
	void AddRequiredResource(TSubclassOf<UDNATaskResource> RequiredResource);
	void AddRequiredResourceSet(const TArray<TSubclassOf<UDNATaskResource> >& RequiredResourceSet);
	void AddRequiredResourceSet(FDNAResourceSet RequiredResourceSet);

	/** 
	 */
	void AddClaimedResource(TSubclassOf<UDNATaskResource> ClaimedResource);
	void AddClaimedResourceSet(const TArray<TSubclassOf<UDNATaskResource> >& AdditionalResourcesToClaim);
	void AddClaimedResourceSet(FDNAResourceSet AdditionalResourcesToClaim);
...
protected:
	...
	static IDNATaskOwnerInterface* ConvertToTaskOwner(UObject& OwnerObject);
	static IDNATaskOwnerInterface* ConvertToTaskOwner(AActor& OwnerActor);

	// protected by design. Not meant to be called outside from DNATaskComponent mechanics
	virtual void Pause();
	virtual void Resume();

	// IDNATaskOwnerInterface BEGIN
...
protected:
	 ...
	 *	provided DNATasksComponent is configured to care about priorities (the default behavior)*/
	uint8 Priority;

	EDNATaskState TaskState;
	...
	FDNAResourceSet RequiredResources;

	/**
	 *	Resources that are going to be locked when this task gets activated, but are not required to get this task started
	 */
	FDNAResourceSet ClaimedResources;

	/** Task Owner that created us */
	TWeakInterfacePtr<IDNATaskOwnerInterface> TaskOwner;

	TWeakObjectPtr<UDNATasksComponent>	TasksComponent;
...
template <class T>
T* UDNATask::NewTask(UObject& WorldContextObject, FName InstanceName)
{
	IDNATaskOwnerInterface* TaskOwner = ConvertToTaskOwner(WorldContextObject);
	return (TaskOwner) ? NewTask<T>(*TaskOwner, InstanceName) : nullptr;
}

template <class T>
T* UDNATask::NewTask(IDNATaskOwnerInterface& TaskOwner, FName InstanceName)
{
	T* MyObj = NewObject<T>();
	MyObj->InstanceName = InstanceName;
	MyObj->InitTask(TaskOwner, TaskOwner.GetDNATaskDefaultPriority());
	return MyObj;
}

//For searching through lists of task instances
struct FDNATaskInstanceNamePredicate
{
	FDNATaskInstanceNamePredicate(FName DesiredInstanceName)
	{
		InstanceName = DesiredInstanceName;
	}

	bool operator()(const UDNATask* A) const
	{
...
struct FDNATaskInstanceClassPredicate
{
	FDNATaskInstanceClassPredicate(TSubclassOf<UDNATask> Class)
	{
		TaskClass = Class;
	}
	...
	TSubclassOf<UDNATask> TaskClass;
};

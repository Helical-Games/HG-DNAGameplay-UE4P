...
ADNAAbilitySystemDebugHUD::ADNAAbilitySystemDebugHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{

}

void ADNAAbilitySystemDebugHUD::DrawWithBackground(UFont* InFont, const FString& Text, const FColor& TextColor, EAlignHorizontal::Type HAlign, float& OffsetX, EAlignVertical::Type VAlign, float& OffsetY, float Alpha)
{
	float SizeX, SizeY;
...
void ADNAAbilitySystemDebugHUD::DrawDebugHUD(UCanvas* InCanvas, APlayerController* PC)
{
	Canvas = InCanvas;
	if (!Canvas)
	{
	...
	if (PC->GetPawn())
	{
		UDNAAbilitySystemComponent * DNAAbilitySystemComponent = PC->GetPawn()->FindComponentByClass<UDNAAbilitySystemComponent>();
		if (DNAAbilitySystemComponent)
		{
			DrawDebugDNAAbilitySystemComponent(DNAAbilitySystemComponent);
		}
	}
}

void ADNAAbilitySystemDebugHUD::DrawDebugDNAAbilitySystemComponent(UDNAAbilitySystemComponent *Component)
{
	...
	for (const UAttributeSet* Set : Component->SpawnedAttributes)
	{
		...
		Y+= 25;
		// Draw Active DNAEffect
		for (FActiveDNAEffect& Effect : &Component->ActiveDNAEffects)
		{
...
static void	ToggleDebugHUD(const TArray<FString>& Args, UWorld* InWorld)
{
	...
	ADNAAbilitySystemDebugHUD *HUD = NULL;

	for (TActorIterator<ADNAAbilitySystemDebugHUD> It(InWorld); It; ++It)
	{
		HUD = *It;
		break;
	}
	...
	if (!HUD)
	{
		HUD = InWorld->SpawnActor<ADNAAbilitySystemDebugHUD>();
		
		FDebugDrawDelegate DrawDebugDelegate = FDebugDrawDelegate::CreateUObject(HUD, &ADNAAbilitySystemDebugHUD::DrawDebugHUD);
		DrawDebugDelegateHandle = UDebugDrawService::Register(TEXT("DNADebug"), DrawDebugDelegate);
	}
	else
	{
		FDebugDrawDelegate DrawDebugDelegate = FDebugDrawDelegate::CreateUObject(HUD, &ADNAAbilitySystemDebugHUD::DrawDebugHUD);
...
FAutoConsoleCommandWithWorldAndArgs DNAAbilitySystemToggleDebugHUDCommand(
	TEXT("DNAAbilitySystem.ToggleDebugHUD"),

...
#include "DNAPrediction.generated.h"

class UDNAAbilitySystemComponent;

DECLARE_DELEGATE(FDNAPredictionKeyEvent);

/**
 *	
 *	Overview of DNA Ability Prediction
 *	
 *	High Level Goals:
 *	At the DNAAbility level (implementing an ability) prediction is transparent. An ability says "Do X->Y->Z", and we will automatically predict the parts of that that we can. 
 ...
 *	-DNAEffect removal
 *	-DNAEffect periodic effects (dots ticking)
 *	
 *	
 ...
 *	*** PredictionKey ***
 *	
 *	A fundamental concept in this system is the Prediction Key (FDNAPredictionKey). A prediction key on its own is simply a unique ID that is generated in a central place on the client. The client will send his prediction key to the server, 
 ...
 *	
 *	(IMPORTANT) FDNAPredictionKey always replicate client -> server, but when replicating server -> clients they *only* replicate to the client that sent the prediction key to the server in the first place. 
 *	This happens in FDNAPredictionKey::NetSerialize. All other clients will receive an invalid (0) prediction key when a prediction key sent from a client is replicated back down through a replicated property.
 *	
 *	
 *	*** Ability Activation ***
 *	
 ...
 *	
 *	
 *	DNAAbilitySystemComponent provides a set of functions for communicating ability activation between clients and server: TryActivateAbility -> ServerTryActivateAbility ->  ClientActivateAbility(Failed/Succeed).
 *	
 *	1. Client calls TryActivateAbility which generates a new FDNAPredictionKey and calls ServerTryActivateAbility.
 ...
 *	3. Any side effects that happen /before the call to ActivatAbility finish/ have the generated FDNAPredictionKey associated with them.
 *	4. Server decides if the ability really happened in ServerTryActivateAbility, calls ClientActivateAbility(Failed/Succeed) and sets UDNAAbilitySystemComponent::ReplicatedPredictionKey to the generated key that was sent.
 ...
 *		5a. 'Rolling back' is accomplished via FDNAPredictionKeyDelegates and FDNAPredictionKey::NewRejectedDelegate/NewCaughtUpDelegate/NewRejectOrCaughtUpDelegate. 
 *
 *
 ...
 *		ThisPredictionKey.NewRejectedDelegate().BindUObject(this, &UDNAAbilitySystemComponent::OnClientActivateAbilityFailed, Handle, ThisPredictionKey.Current);
 *		
 *		
 *		Invoking the callback in ClientActivateAbilityFailed_Implementation:
 *		FDNAPredictionKeyDelegates::BroadcastRejectedDelegate(PredictionKey);
 *		
 ...
 *		key used previous steps, the client can undo his predictive side effects. See UDNAAbilitySystemComponent::OnRep_PredictionKey.
 *						
 *			 
 *			 
 *			 
 *	*** DNAEffect Prediction ***
 *
 *	DNAEffects are considered side effects of prediction and are not explicitly asked about.
 *	
 *	1. DNAEffects are only applied on clients if there is a valid prediction key. (If no prediction key, it simply skips the application on client).
 *	2. Attributes, DNACues, and DNATags are all predicted if the DNAEffect is predicted.
 *	3. When the FActiveDNAEffect is created, it stores the prediction key (FActiveDNAEffect::PredictionKey)
 *		3a. Instant effects are explained below in "Attribute Prediction".
 *	4. On the server, the same prediction key is also set on the server's FActiveDNAEffect that will be replicated down.
 *	5. As a client, if you get a replicated FActiveDNAEffect with a valid prediction key on it, you check to see if you have an ActiveDNAEffect with that same key, if there is match, we do not apply
 *		the 'on applied' type of logic, e.g., DNACues. The solves the "Redo" problem. However we will have 2 of the 'same' DNAEffects in our ActiveDNAEffects container, temporarily:
 *	6. At the same time, UDNAAbilitySystemComponent::ReplicatedPredictionKey will catch up and the predictive effects will be removed. When they are removed in this case, we again check PredicitonKey and decide 
 *		if we should not do the 'On Remove' logic / DNACue.
 *		
 *	At this point, we have effectively predicted a DNA effect as a side effect and handled the 'Undo' and 'Redo' problems.
 *		
 *	
 ...
 *	
 *	
 *	1. We treat predictive instant DNA effects as infinite duration gamepaly effects. See UDNAAbilitySystemComponent::ApplyDNAEffectSpecToSelf.
 ...
 *	3. In the attribute RepNotify, we call into the DNAAbilitySystemComponent::ActiveDNAEffects to update our 'final value' give the new 'base value'. the DNAATTRIBUTE_REPNOTIFY can do this.
 *	4. Everything else will work like above (DNAEffect prediction) : when the prediction key is caught up, the predictive DNAEffect is removed and we will return to the server given value.
 *	
 *	
 *	Example:
 *	
 ...
 *  *** DNA Cue Events ***
 *  
 *  Outside of DNAEffects which are already explained, DNA Cues can be activated on their own. These functions (UDNAAbilitySystemComponent::ExecuteDNACue etc)  take network role and prediction keys into account.
 *  
 *  1. In UDNAAbilitySystemComponent::ExecuteDNACue, if authority then do the multicast event (with replication key). If non authority but w/ a valid prediction key, predict the DNACue.
 *  2. On the receiving end (NetMulticast_InvokeDNACueExecuted etc), if there is a replication key, then don't do the event (assume you predicted it).
 *  
 *  Remember that FDNAPredictionKeys only replicate to the originating owner. This is an intrinsic property of FReplicationKey.
 *	
 ...
 *	
 *	To handle this, we have a concept of a Base PredictionKey, which is a member of FDNAPredictionKey. When calling TryActivateAbility, we pass in the current PredictionKey (if applicable). That prediction key
 ...
 *	is not logically atomic. Each side effect produced in the chain of events has to have a unique PredictionKey. We cannot have DNAEffects produced in X have the same PredictionKey produced in Z.
 *	
 *	To get around this, The prediction key of X is considered the Base key for Y and Z. The dependancy from Y to Z is kept completely client side, which is done in by FDNAPredictionKeyDelegates::AddDependancy. We add delegates
 ...
 *	
 *	UDNAAbilityTask_WaitInputRelease::OnReleaseCallback is a good example. The flow of events is as followed:
 *	1. Client enters UDNAAbilityTask_WaitInputRelease::OnReleaseCallback and starts a new FScopedPredictionWindow. This creates a new prediction key for this scope (FScopedPredictionWindow::ScopedPredictionKey).
 *	2. Client calls DNAAbilitySystemComponent->ServerInputRelease which passes ScopedPrediction.ScopedPredictionKey as a parameter.
 *	3. Server runs ServerInputRelease_Implementation which takes the passed in PredictionKey and sets it as UDNAAbilitySystemComponent::ScopedPredictionKey with an FScopedPredictionWindow.
 *	4. Server runs UDNAAbilityTask_WaitInputRelease::OnReleaseCallback /within the same scope/
 *	5. When the server hits the FScopedPredictionWindow in ::OnReleaseCallback, it gets the prediction key from UDNAAbilitySystemComponent::ScopedPredictionKey. That is now used for all side effects within this logical scope.
 ...
 *	for this should eventually be added and it should follow the same pattern that DNAEffect and DNACues follow (predict triggered event with a prediction key, ignore the RPC event if it has a prediction key).
 *	
 *		 
 *	*** Predicting "Meta" Attributes such as Damage/Healing vs "real" attributes such as Health ***
 *	
 *	We are unable to apply meta attributes predictively. Meta attributes only work on instant effects, in the back end of DNAEffect (Pre/Post Modify Attribute on the UDNAAttributeSet). These events are not called when
 *	applying duration-based DNA effects. E.g., a DNAEffect that modifies damage for 5 seconds doesn't make sense. 
 *	
 *	In order to support this, we would probably add some limited support for duration based meta attributes, and move the transform of the instant DNA effect from the front end (UDNAAbilitySystemComponent::ApplyDNAEffectSpecToSelf)
 *	to the backend (UDNAAttributeSet::PostModifyAttribute). 
 * 
 *			 
 *	*** Predicting ongoing multiplicitive DNAEffects ***
 *	
 *	There are also limitations when predicting % based DNA effects. Since the server replicates down the 'final value' of an attribute, but not the entire aggregator chain of what is modifying it, we may run into cases where
 *	the client cannot accurately predict new DNA effects.
 *	
 *	For example: 
 ...
 *	receives a DNAEffect that slows them and their material blue. Since we can't send Server RPCs every time this happens (and the server couldn't necessarily handle the message at his point in the simulation), there is no 
 *	way to correlate the DNA effect side effects between client and server.
 *	
 ...
 *	only certain actions can be predicted - for example DNACue execute events, but not OnAdded/OnRemove events.
 *	
 *	
 */
...
/**
 *	FDNAPredictionKey is a generic way of supporting Clientside Prediction in the DNAAbility system.
 *	A FDNAPredictionKey is essentially an ID for identifying predictive actions and side effects that are
 *	done on a client. UDNAAbilitySystemComponent supports synchronization of the prediction key and its side effects
 ...
 *	
 *	While the client is predictively his ability, he is creating side effects (DNAEffects, TriggeredEvents, Animations, etc).
 ...
 *		
 *	The main things FDNAPredictionKey itself provides are:
...
USTRUCT()
struct DNAABILITIES_API FDNAPredictionKey
{
	...
	static FDNAPredictionKey CreateNewPredictionKey(class UDNAAbilitySystemComponent*);

	/** Construct a new server initiation key, for abilities activated on the server */
	static FDNAPredictionKey CreateNewServerInitiatedKey(class UDNAAbilitySystemComponent*);
	...
	bool operator==(const FDNAPredictionKey& Other) const
	{
	...
	friend uint32 GetTypeHash(const FDNAPredictionKey& InKey)
	{
...
template<>
struct TStructOpsTypeTraits<FDNAPredictionKey> : public TStructOpsTypeTraitsBase
{
	enum
	{
...
struct FDNAPredictionKeyDelegates
{
...
public:
	...
	public:
		...
		TArray<FDNAPredictionKeyEvent>	RejectedDelegates;

		/** This delegate is called when replicated state has caught up with the prediction key. Doesnt imply rejection or acceptance. */
		TArray<FDNAPredictionKeyEvent>	CaughtUpDelegates;
	};

	TMap<FDNAPredictionKey::KeyType, FDelegates>	DelegateMap;

	static FDNAPredictionKeyDelegates& Get();

	static FDNAPredictionKeyEvent&	NewRejectedDelegate(FDNAPredictionKey::KeyType Key);
	static FDNAPredictionKeyEvent&	NewCaughtUpDelegate(FDNAPredictionKey::KeyType Key);
	static void NewRejectOrCaughtUpDelegate(FDNAPredictionKey::KeyType Key, FDNAPredictionKeyEvent NewEvent);

	static void	BroadcastRejectedDelegate(FDNAPredictionKey::KeyType Key);
	static void	BroadcastCaughtUpDelegate(FDNAPredictionKey::KeyType Key);

	static void Reject(FDNAPredictionKey::KeyType Key);
	static void CatchUpTo(FDNAPredictionKey::KeyType Key);

	static void AddDependency(FDNAPredictionKey::KeyType ThisKey, FDNAPredictionKey::KeyType DependsOn);

private:

	static void CaughtUp(FDNAPredictionKey::KeyType Key);
};
...
struct DNAABILITIES_API FScopedPredictionWindow
{
	 ...
	 **/
	FScopedPredictionWindow(UDNAAbilitySystemComponent* DNAAbilitySystemComponent, FDNAPredictionKey InPredictionKey, bool InSetReplicatedPredictionKey = true);
	...
	FScopedPredictionWindow(UDNAAbilitySystemComponent* DNAAbilitySystemComponent, bool CanGenerateNewKey=true);

	~FScopedPredictionWindow();

	FDNAPredictionKey	ScopedPredictionKey;

private:

	TWeakObjectPtr<UDNAAbilitySystemComponent> Owner;
	bool ClearScopedPredictionKey;
	bool SetReplicatedPredictionKey;
	FDNAPredictionKey RestoreKey;
};

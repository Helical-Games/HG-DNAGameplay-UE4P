...
/*================================================================================
ActiveDNAEffectIterator.h: implementation of ActiveDNAEffect Iterator.

The is an iterator for FActiveDNAEffectContainer. The main points are:
-This iterates over both the active DNA effect array and the pending list of new GEs
-This skips any DNA effects which are pending remove
-This locks the array while the iterator is in scope, meaning any adds or removes 
 to the active DNA effect array will have their memory operations deffered.
...
class FActiveDNAEffectIterator
{
public:

	FActiveDNAEffectIterator(const ContainerType& InContainer, int32 StartIdx=0)
		: index(StartIdx)
		, Current(nullptr)
		, Pending(nullptr)
...
private:
	...
	{
		// Advance pending next only if this isnt the tail of the pending DNA effect list
		return (Next != const_cast<ElementType**>(Container.PendingDNAEffectNext)) ? *Next : nullptr;
	}
	...
	void UpdateCurrent()
	{
		...
		else if (index >= Container.DNAEffects_Internal.Num())
		{
			// Once we get to end of the array, we start iterating on the linked list. 
			Pending = AdvancePending(const_cast<ElementType**>(&Container.PendingDNAEffectHead));
			Current = Pending;
			index = INDEX_NONE;
		}
		...
		else
		{
			...
			Current = &Container.DNAEffects_Internal[index];
		}

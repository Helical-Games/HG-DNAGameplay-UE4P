...
#include "DNAEffect.h"
#include "DNAModMagnitudeCalculation.generated.h"

/** Class used to perform custom DNA effect modifier calculations, either via blueprint or native code */ 
UCLASS(BlueprintType, Blueprintable, Abstract)
class DNAABILITIES_API UDNAModMagnitudeCalculation : public UDNAEffectCalculation
{
...
public:
	...
	/**
	 * Calculate the base magnitude of the DNA effect modifier, given the specified spec. Note that the owning spec def can still modify this base value
	 * with a coeffecient and pre/post multiply adds. see FCustomCalculationBasedFloat::CalculateMagnitude for details.
	 * 
	 * @param Spec	DNA effect spec to use to calculate the magnitude with
	 * 
	 * @return Computed magnitude of the modifier
	 */
	UFUNCTION(BlueprintNativeEvent, Category="Calculation")
	float CalculateBaseMagnitude(const FDNAEffectSpec& Spec) const;
	 ...
	 * and updated.
	 * 
	 * @param Spec	DNA effect spec that is requesting the delegate
	 ...
	 */
	virtual FOnExternalDNAModifierDependencyChange* GetExternalModifierDependencyMulticast(const FDNAEffectSpec& Spec, UWorld* World) const;
...
protected:
	...
	bool GetCapturedAttributeMagnitude(const FDNAEffectAttributeCaptureDefinition& Def, const FDNAEffectSpec& Spec, const FAggregatorEvaluateParameters& EvaluationParameters, OUT float& Magnitude) const;
	 ...
	 *			that are utilizing network dormancy and may want to trust the client to update non-DNA critical attributes client-side without

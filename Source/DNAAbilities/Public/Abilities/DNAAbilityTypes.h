...
#include "DNAAbilityTypes.generated.h"
...
DNAABILITIES_API DECLARE_LOG_CATEGORY_EXTERN(LogDNAAbilitySystemComponent, Log, All);

#define ENABLE_ABILITYTASK_DEBUGMSG !(UE_BUILD_SHIPPING | UE_BUILD_TEST)

UENUM(BlueprintType)
namespace EDNAAbilityInstancingPolicy
{
	/**
...
UENUM(BlueprintType)
namespace EDNAAbilityNetExecutionPolicy
{
	/**
...
UENUM(BlueprintType)
namespace EDNAAbilityReplicationPolicy
{
	/**
...
UENUM(BlueprintType)
namespace EDNAAbilityTriggerSource
{
	/**	Defines what type of trigger will activate the ability, paired to a tag */

	enum Type
	{
		// Triggered from a DNA event, will come with payload
		DNAEvent,
 ...
 *		-Projects can override UDNAAbilitySystemGlobals::AllocAbilityActorInfo to override the default struct type that is created.
 *
 */
USTRUCT(BlueprintType)
struct DNAABILITIES_API FDNAAbilityActorInfo
{
	GENERATED_USTRUCT_BODY()

	virtual ~FDNAAbilityActorInfo() {}
	...
	UPROPERTY(BlueprintReadOnly, Category = "ActorInfo")
	TWeakObjectPtr<UDNAAbilitySystemComponent>	DNAAbilitySystemComponent;
	...
	virtual void InitFromActor(AActor *OwnerActor, AActor *AvatarActor, UDNAAbilitySystemComponent* InDNAAbilitySystemComponent);
...
USTRUCT()
struct FDNAAbilitySpecHandleAndPredictionKey
{
	GENERATED_USTRUCT_BODY()

	FDNAAbilitySpecHandleAndPredictionKey()
	: PredictionKeyAtCreation(0)
	{}

	FDNAAbilitySpecHandleAndPredictionKey(const FDNAAbilitySpecHandle& HandleRef, const FPredictionKey& PredictionKeyAtCreationRef)
		: AbilityHandle(HandleRef), PredictionKeyAtCreation(PredictionKeyAtCreationRef.Current)
	{}

	bool operator==(const FDNAAbilitySpecHandleAndPredictionKey& Other) const
	{
	...
	friend uint32 GetTypeHash(const FDNAAbilitySpecHandleAndPredictionKey& Handle)
	{
...
private:
	...
	UPROPERTY()
	FDNAAbilitySpecHandle AbilityHandle;
...
USTRUCT()
struct DNAABILITIES_API FDNAAbilityRepAnimMontage
{
	GENERATED_USTRUCT_BODY()
...
USTRUCT()
struct DNAABILITIES_API FDNAAbilityLocalAnimMontage
{
	GENERATED_USTRUCT_BODY()

	FDNAAbilityLocalAnimMontage()
	...
	UPROPERTY()
	UDNAAbility* AnimatingAbility;
};

// ----------------------------------------------------

USTRUCT(BlueprintType)
struct DNAABILITIES_API FDNAEventData
{
	GENERATED_USTRUCT_BODY()
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = DNAAbilityTriggerPayload)
	FDNATag EventTag;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = DNAAbilityTriggerPayload)
	FDNAEffectContextHandle ContextHandle;

	// Tags that the instigator has
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = DNAAbilityTriggerPayload)
	FDNATagContainer InstigatorTags;

	// Tags that the target has
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = DNAAbilityTriggerPayload)
	FDNATagContainer TargetTags;
...
DECLARE_MULTICAST_DELEGATE_OneParam(FDNAEventMulticastDelegate, const FDNAEventData*);

/** 
 *	Structure that tells DNAAbilitySystemComponent what to bind to an InputComponent (see BindAbilityActivationToInputComponent) 
 *	
 */
struct FDNAAbiliyInputBinds
{
	FDNAAbiliyInputBinds(FString InConfirmTargetCommand, FString InCancelTargetCommand, FString InEnumName, int32 InConfirmTargetInputID = INDEX_NONE, int32 InCancelTargetInputID = INDEX_NONE)
...
USTRUCT()
struct DNAABILITIES_API FAttributeDefaults
{
	GENERATED_USTRUCT_BODY()

	UPROPERTY(EditAnywhere, Category = "AttributeTest")
	TSubclassOf<UDNAAttributeSet> Attributes;
...
USTRUCT()
struct DNAABILITIES_API FDNAAbilityTaskDebugMessage
{
	GENERATED_USTRUCT_BODY()

	UPROPERTY()
	class UDNATask*	FromTask;

	FString Message;
};

/** Used for cleaning up predicted data on network clients */
DECLARE_MULTICAST_DELEGATE(FDNAAbilitySystemComponentPredictionKeyClear);

/** Generic delegate for ability 'events'/notifies */
DECLARE_MULTICAST_DELEGATE_OneParam(FGenericAbilityDelegate, UDNAAbility*);

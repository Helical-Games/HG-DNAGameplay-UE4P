 ...
 * - The ability itself ends via ADNAAbility::EndAbility
 * - The ability state is manually ended via ADNAAbility::EndAbilityState
 * - Another ability state is started will 'bEndCurrentState' set to true
 *
 * 'OnStateInterrupted' will be called if:
 * - The ability itself is cancelled via ADNAAbility::CancelAbility
 */
UCLASS(MinimalAPI)
class UDNAAbilityTask_StartAbilityState : public UDNAAbilityTask
{
	...
	static UDNAAbilityTask_StartAbilityState* StartAbilityState(UDNAAbility* OwningAbility, FName StateName, bool bEndCurrentState = true);

...
struct DNAABILITIES_API FAggregatorEvaluateParameters
{
	...
	const FDNATagContainer* SourceTags;
	const FDNATagContainer* TargetTags;

	/** Any mods with one of these handles will be ignored during evaluation */
	TArray<FActiveDNAEffectHandle> IgnoreHandles;

	/** If any tags are specified in the filter, a mod's owning active DNA effect's source tags must match ALL of them in order for the mod to count during evaluation */
	FDNATagContainer AppliedSourceTagFilter;

	/** If any tags are specified in the filter, a mod's owning active DNA effect's target tags must match ALL of them in order for the mod to count during evaluation */
	FDNATagContainer AppliedTargetTagFilter;

	bool IncludePredictiveMods;
};

struct DNAABILITIES_API FAggregatorMod
{
	const FDNATagRequirements*	SourceTagReqs;
	const FDNATagRequirements*	TargetTagReqs;

	float EvaluatedMagnitude;		// Magnitude this mod was last evaluated at
	float StackCount;

	FActiveDNAEffectHandle ActiveHandle;	// Handle of the active DNAEffect we are tied to (if any)
	bool IsPredicted;
...
struct DNAABILITIES_API FAggregatorModChannel
{
	/**
	 ...
	 * @param ActiveHandle			Handle of the active DNA effect that's applying the mod
	 */
	void AddMod(float EvaluatedMagnitude, TEnumAsByte<EDNAModOp::Type> ModOp, const FDNATagRequirements* SourceTagReqs, const FDNATagRequirements* TargetTagReqs, bool bIsPredicted, const FActiveDNAEffectHandle& ActiveHandle);
	
	/**
	 * Remove all mods from the channel that match the specified DNA effect handle
	 * 
	 * @param Handle	Handle to use for removal
	 */
	void RemoveModsWithActiveHandle(const FActiveDNAEffectHandle& Handle);
	 ...
	 */
	void DebugGetAllAggregatorMods(EDNAModEvaluationChannel Channel, OUT TMap<EDNAModEvaluationChannel, const TArray<FAggregatorMod>*>& OutMods) const;
	
	/**
	 * Called when the mod channel's DNA effect dependencies have potentially been swapped out for new ones, like when GE arrays are cloned.
	 * Updates mod handles appropriately.
	 * 
	 * @param SwappedDependencies	Mapping of old DNA effect handles to new replacements
	 */
	void OnActiveEffectDependenciesSwapped(const TMap<FActiveDNAEffectHandle, FActiveDNAEffectHandle>& SwappedDependencies);
...
private:
	...
	TArray<FAggregatorMod> Mods[EDNAModOp::Max];
};

/** Struct representing a container of modifier channels */
struct DNAABILITIES_API FAggregatorModChannelContainer
{
	/**
	 ...
	 */
	FAggregatorModChannel& FindOrAddModChannel(EDNAModEvaluationChannel Channel);
	 ...
	 * @return Result of the specified base value run through each modifier in each evaluation channel in numeric order
	 */
	float EvaluateWithBaseToChannel(float InlineBaseValue, const FAggregatorEvaluateParameters& Parameters, EDNAModEvaluationChannel FinalChannel) const;
	 ...
	 */
	void RemoveAggregatorMod(const FActiveDNAEffectHandle& ActiveHandle);
	 ...
	 * @param OutMods	Mapping of channel enum to mods
	 */
	void DebugGetAllAggregatorMods(OUT TMap<EDNAModEvaluationChannel, const TArray<FAggregatorMod>*>& OutMods) const;

	/**
	 * Called when the container's DNA effect dependencies have potentially been swapped out for new ones, like when GE arrays are cloned.
...
private:
	...
	TMap<EDNAModEvaluationChannel, FAggregatorModChannel> ModChannelsMap;
};

struct DNAABILITIES_API FAggregator : public TSharedFromThis<FAggregator>
{
	...
	void ExecModOnBaseValue(TEnumAsByte<EDNAModOp::Type> ModifierOp, float EvaluatedMagnitude);
	static float StaticExecModOnBaseValue(float BaseValue, TEnumAsByte<EDNAModOp::Type> ModifierOp, float EvaluatedMagnitude);

	void AddAggregatorMod(float EvaluatedData, TEnumAsByte<EDNAModOp::Type> ModifierOp, EDNAModEvaluationChannel ModifierChannel, const FDNATagRequirements*	SourceTagReqs, const FDNATagRequirements* TargetTagReqs, bool IsPredicted, FActiveDNAEffectHandle ActiveHandle = FActiveDNAEffectHandle() );

	/** Removes all mods for the passed in handle and marks this as dirty to recalculate the aggregator */
	void RemoveAggregatorMod(FActiveDNAEffectHandle ActiveHandle);
	...
	void UpdateAggregatorMod(FActiveDNAEffectHandle ActiveHandle, const FDNAAttribute& Attribute, const FDNAEffectSpec& Spec, bool bWasLocallyGenerated, FActiveDNAEffectHandle InHandle);
	...
	float EvaluateToChannel(const FAggregatorEvaluateParameters& Parameters, EDNAModEvaluationChannel FinalChannel) const;
	...
	float EvaluateContribution(const FAggregatorEvaluateParameters& Parameters, FActiveDNAEffectHandle ActiveHandle) const;
	...
	void AddDependent(FActiveDNAEffectHandle Handle);
	void RemoveDependent(FActiveDNAEffectHandle Handle);
	...
	/**
	 * Called when the aggregator's DNA effect dependencies have potentially been swapped out for new ones, like when GE arrays are cloned.
...
private:
	...
	TArray<FActiveDNAEffectHandle>	Dependents;
	...
	friend class UDNAAbilitySystemComponent;	// Only needed for DisplayDebug()
};

struct DNAABILITIES_API FAggregatorRef
{
	FAggregatorRef() { }
 ...
 */
struct DNAABILITIES_API FScopedAggregatorOnDirtyBatch
{

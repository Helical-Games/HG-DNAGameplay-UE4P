...
//////////////////////////////////////////////////////////////////////////
// FDNADebuggerCategoryReplicatorData

DEFINE_LOG_CATEGORY_STATIC(LogDNADebugReplication, Display, All);

static TAutoConsoleVariable<int32> CVarDNADebuggerRepDetails(
	TEXT("ai.debug.DetailedReplicationLogs"),
	0,
	TEXT("Enable or disable very verbose replication logs for DNA debugger"),
	ECVF_Cheat);
...
public:
	struct FDataPackState
	{
		...
		FDataPackState(const FDNADebuggerDataPack::FHeader& Header) : DataOffset(Header.DataOffset), DataVersion(Header.DataVersion), SyncCounter(Header.SyncCounter) {}
	...
	void DumpToLog()
	{
		for (int32 CategoryIdx = 0; CategoryIdx < CategoryStates.Num(); CategoryIdx++)
		{
			...
			UE_LOG(LogDNADebugReplication, Verbose, TEXT("category[%d] TextLinesRepCounter:%d ShapesRepCounter:%d"),
			...
			for (int32 DataPackIdx = 0; DataPackIdx < CategoryData.DataPackStates.Num(); DataPackIdx++)
			{
				...
				UE_LOG(LogDNADebugReplication, Verbose, TEXT(">>    data[%d] DataVersion:%d SyncCounter:%d DataOffset:%d"),
...
bool FDNADebuggerNetPack::NetDeltaSerialize(FNetDeltaSerializeInfo& DeltaParms)
{
	...
	if (DeltaParms.Writer)
	{
		...
		if (Owner->bIsEnabled && Owner->Categories.Num() == SavedData.Num())
		{
			...
			*DeltaParms.NewState = TSharedPtr<INetDeltaBaseState>(NewState);
			...
			for (int32 Idx = 0; Idx < SavedData.Num(); Idx++)
			{
				...
				FDNADebuggerCategory& CategoryOb = Owner->Categories[Idx].Get();
				...
				{
					FDNADebuggerDataPack& DataPack = CategoryOb.ReplicatedDataPacks[DataIdx];
					...
					if (DataPack.bNeedsConfirmation && !DataPack.bReceived && bHasOldStatePack)
					{
						...
						UE_LOG(LogDNADebugReplication, Verbose, TEXT("Checking packet confirmation for Category[%d].DataPack[%d] OldState(DataVersion:%d DataOffset:%d complete:%s) current(DataVersion:%d DataOffset:%d)"),
							Idx, DataIdx,
		...
		if (CVarDNADebuggerRepDetails.GetValueOnAnyThread())
		{
			if (OldState)
			{
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("NetDeltaSerialize DUMP OldState"));
				OldState->DumpToLog();
			}
			if (NewState)
			{
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("NetDeltaSerialize DUMP NewState"));
				NewState->DumpToLog();
			}
		}
		...
		UE_LOG(LogDNADebugReplication, Verbose, TEXT("NetDeltaSerialize WRITE START, OldState:%d"), OldState ? 1 : 0);
		for (int32 Idx = 0; Idx < SavedData.Num(); Idx++)
		{
			FDNADebuggerCategory& CategoryOb = Owner->Categories[Idx].Get();
			const bool bMissingOldState = (OldState == nullptr) || !OldState->CategoryStates.IsValidIndex(Idx);
			...
			{
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("  CATEGORY[%d]:%s"), Idx, *CategoryOb.GetCategoryName().ToString());
			}

			if (ShouldUpdateTextLines)
			{
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("  >> replicate lines"));
				Writer << SavedCategory.TextLines;
			}

			if (ShouldUpdateShapes)
			{
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("  >> replicate shapes"));
			...
			for (int32 DataIdx = 0; DataIdx < NumDataPacks; DataIdx++)
			{
				...
				if (ShouldUpdateDataPack)
				{
					...
					const int32 PacketSize = FMath::Min(FDNADebuggerDataPack::PacketSize, DataPack.Header.DataSize - DataPack.Header.DataOffset);
					if (PacketSize > 0)
					{
						Writer.Serialize(DataPack.Data.GetData() + DataPack.Header.DataOffset, PacketSize);
					}

					UE_LOG(LogDNADebugReplication, Verbose, TEXT("  >> replicate data pack[%d] progress:%0.f%% (offset:%d packet:%d)"),
	...
	else if (DeltaParms.Reader)
	{
		...
		UE_LOG(LogDNADebugReplication, Verbose, TEXT("NetDeltaSerialize READ START"));
		...
		{
			UE_LOG(LogDNADebugReplication, Error, TEXT("Category count mismtach! received:%d expected:%d"), CategoryCount, Owner->Categories.Num());
			Reader.SetError();
			return false;
		}
		...
		for (int32 Idx = 0; Idx < CategoryCount; Idx++)
		{
			...
			{
				UE_LOG(LogDNADebugReplication, Error, TEXT("Data pack count mismtach! received:%d expected:%d"), NumDataPacks, CategoryOb.ReplicatedDataPacks.Num());
				Reader.SetError();
				return false;
			}
			...
			if (ShouldUpdateTextLines)
			{
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("  >> received lines"));
				Reader << CategoryOb.ReplicatedLines;
			}

			if (ShouldUpdateShapes)
			{
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("  >> received shapes"));
			...
			for (uint8 DataIdx = 0; DataIdx < NumDataPacks; DataIdx++)
			{
				...
				if (ShouldUpdateDataPack)
				{
					...
					FDNADebuggerDataPack DataPacket;
					...
					const int32 PacketSize = FMath::Min(FDNADebuggerDataPack::PacketSize, DataPacket.Header.DataSize - DataPacket.Header.DataOffset);
					if (PacketSize > 0)
					{
					...
					UE_LOG(LogDNADebugReplication, Verbose, TEXT("  >> replicate data pack[%d] progress:%.0f%%"), DataIdx, CategoryOb.ReplicatedDataPacks[DataIdx].GetProgress() * 100.0f);
				}
			}
		}
	}
...
void FDNADebuggerNetPack::OnCategoriesChanged()
{
	SavedData.Reset();
...
//////////////////////////////////////////////////////////////////////////
// ADNADebuggerCategoryReplicator

ADNADebuggerCategoryReplicator::ADNADebuggerCategoryReplicator(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
void ADNADebuggerCategoryReplicator::BeginPlay()
{
	Super::BeginPlay();
	...
	AddonManager.OnCategoriesChanged.AddUObject(this, &ADNADebuggerCategoryReplicator::OnCategoriesChanged);
	AddonManager.OnExtensionsChanged.AddUObject(this, &ADNADebuggerCategoryReplicator::OnExtensionsChanged);

	OnCategoriesChanged();
	OnExtensionsChanged();

	ADNADebuggerPlayerManager& PlayerManager = ADNADebuggerPlayerManager::GetCurrent(GetWorld());
	PlayerManager.RegisterReplicator(*this);

	SetActorHiddenInGame(!bIsLocal);
	if (bIsLocal)
	{
		RenderingComp = NewObject<UDNADebuggerRenderingComponent>(this, TEXT("RenderingComp"));
	...
	if (bHasAuthority)
	{
		SetEnabled(FDNADebuggerAddonBase::IsSimulateInEditor());
	}
}

void ADNADebuggerCategoryReplicator::Destroyed()
{
	Super::Destroyed();
...
void ADNADebuggerCategoryReplicator::OnCategoriesChanged()
{
	FDNADebuggerAddonManager& AddonManager = FDNADebuggerAddonManager::GetCurrent();
...
void ADNADebuggerCategoryReplicator::OnExtensionsChanged()
{
	FDNADebuggerAddonManager& AddonManager = FDNADebuggerAddonManager::GetCurrent();
...
UNetConnection* ADNADebuggerCategoryReplicator::GetNetConnection() const
{
	return IsValid(OwnerPC) ? OwnerPC->GetNetConnection() : nullptr;
}

bool ADNADebuggerCategoryReplicator::IsNetRelevantFor(const AActor* RealViewer, const AActor* ViewTarget, const FVector& SrcLocation) const
{
	return (RealViewer == OwnerPC);
}

void ADNADebuggerCategoryReplicator::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(ADNADebuggerCategoryReplicator, OwnerPC);
	DOREPLIFETIME(ADNADebuggerCategoryReplicator, DebugActor);
	DOREPLIFETIME(ADNADebuggerCategoryReplicator, bIsEnabled);
	DOREPLIFETIME(ADNADebuggerCategoryReplicator, ReplicatedData);
}

bool ADNADebuggerCategoryReplicator::ServerSetEnabled_Validate(bool bEnable)
{
	return true;
}

void ADNADebuggerCategoryReplicator::ServerSetEnabled_Implementation(bool bEnable)
{
	SetEnabled(bEnable);
}

bool ADNADebuggerCategoryReplicator::ServerSetDebugActor_Validate(AActor* Actor)
{
	return true;
}

void ADNADebuggerCategoryReplicator::ServerSetDebugActor_Implementation(AActor* Actor)
{
	SetDebugActor(Actor);
}

bool ADNADebuggerCategoryReplicator::ServerSetCategoryEnabled_Validate(int32 CategoryId, bool bEnable)
{
	return true;
}

void ADNADebuggerCategoryReplicator::ServerSetCategoryEnabled_Implementation(int32 CategoryId, bool bEnable)
{
	SetCategoryEnabled(CategoryId, bEnable);
}

bool ADNADebuggerCategoryReplicator::ServerSendCategoryInputEvent_Validate(int32 CategoryId, int32 HandlerId)
{
	return true;
}

void ADNADebuggerCategoryReplicator::ServerSendCategoryInputEvent_Implementation(int32 CategoryId, int32 HandlerId)
{
	SendCategoryInputEvent(CategoryId, HandlerId);
}

bool ADNADebuggerCategoryReplicator::ServerSendExtensionInputEvent_Validate(int32 ExtensionId, int32 HandlerId)
{
	return true;
}

void ADNADebuggerCategoryReplicator::ServerSendExtensionInputEvent_Implementation(int32 ExtensionId, int32 HandlerId)
{
	SendExtensionInputEvent(ExtensionId, HandlerId);
}

void ADNADebuggerCategoryReplicator::OnReceivedDataPackPacket(int32 CategoryId, int32 DataPackId, const FDNADebuggerDataPack& DataPacket)
{
	if (Categories.IsValidIndex(CategoryId) && Categories[CategoryId]->ReplicatedDataPacks.IsValidIndex(DataPackId))
	{
		FDNADebuggerDataPack& DataPack = Categories[CategoryId]->ReplicatedDataPacks[DataPackId];
		...
		if (DataPack.Header.DataVersion != DataPacket.Header.DataVersion)
		{
			...
			else
			{
				...
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("Error: received packet from the middle of content with different version, discarding! (category[%d]:%s, packet.DataVersion:%d packet.DataOffset:%d, data[%d].DataVersion:%d)"),
		...
		else if (DataPack.Data.Num() < DataPacket.Header.DataSize)
		{
			...
			else
			{
				...
				UE_LOG(LogDNADebugReplication, Verbose, TEXT("Error: received packet doesn't match expected chunk, discarding! (category[%d]:%s, packet.DataOffset:%d, data[%d].DataOffset:%d data[%d].Data.Num:%d)"),
					CategoryId, *Categories[CategoryId]->GetCategoryName().ToString(),
					DataPacket.Header.DataOffset,
					DataPackId, DataPack.Header.DataOffset,
		...
		{
			// complete
			UE_LOG(LogDNADebugReplication, Verbose, TEXT("Category[%d].DataPack[%d] RECEIVED, DataVersion:%d DataSize:%d SyncCounter:%d"),
...
void ADNADebuggerCategoryReplicator::TickActor(float DeltaTime, enum ELevelTick TickType, FActorTickFunction& ThisTickFunction)
{
	Super::TickActor(DeltaTime, TickType, ThisTickFunction);
	CollectCategoryData();
}

void ADNADebuggerCategoryReplicator::CollectCategoryData(bool bForce)
{
	const float GameTime = GetWorld()->GetTimeSeconds();
	for (int32 Idx = 0; Idx < Categories.Num(); Idx++)
	{
		FDNADebuggerCategory& CategoryOb = Categories[Idx].Get();
		if (CategoryOb.bHasAuthority && CategoryOb.bIsEnabled && (bForce || (GameTime - CategoryOb.LastCollectDataTime) > CategoryOb.CollectDataInterval))
		{
			...
			{
				FDNADebuggerDataPack& DataPack = CategoryOb.ReplicatedDataPacks[DataPackIdx];
				DataPack.bIsDirty = false;

				if ((DataPack.Flags == EDNADebuggerDataPack::ResetOnTick) ||
					(DataPack.Flags == EDNADebuggerDataPack::ResetOnActorChange && DataPack.Header.SyncCounter != DebugActor.SyncCounter))
				{
			...
			for (int32 DataPackIdx = 0; DataPackIdx < CategoryOb.ReplicatedDataPacks.Num(); DataPackIdx++)
			{
				...
				else
				{
					...
					if (bWasDirty)
					{
						UE_LOG(LogDNADebugReplication, Verbose, TEXT("Category[%d].DataPack[%d] SENT, DataVersion:%d DataSize:%d SyncCounter:%d"),
...
void ADNADebuggerCategoryReplicator::SetReplicatorOwner(APlayerController* InOwnerPC)
{
	if (!bIsEnabled)
	{
...
void ADNADebuggerCategoryReplicator::SetEnabled(bool bEnable)
{
	if (bHasAuthority)
	{
		bIsEnabled = bEnable;
	...
	const bool bEnableExtensions = bEnable && !FDNADebuggerAddonBase::IsSimulateInEditor();
	NotifyExtensionsToolState(bEnableExtensions);
}

void ADNADebuggerCategoryReplicator::SetDebugActor(AActor* Actor)
{
	UE_LOG(LogDNADebugReplication, Log, TEXT("SetDebugActor %s"), *GetNameSafe(Actor));
	if (bHasAuthority)
	{
...
void ADNADebuggerCategoryReplicator::SetCategoryEnabled(int32 CategoryId, bool bEnable)
{
	if (bHasAuthority)
	{
		if (Categories.IsValidIndex(CategoryId))
		{
			UE_LOG(LogDNADebugReplication, Log, TEXT("SetCategoryEnabled[%d]:%d (%s)"), CategoryId, bEnable ? 1 : 0, *Categories[CategoryId]->GetCategoryName().ToString());
...
void ADNADebuggerCategoryReplicator::SendCategoryInputEvent(int32 CategoryId, int32 HandlerId)
{
		...
		HandlerId < Categories[CategoryId]->GetNumInputHandlers())
	{
		...
		{
			FDNADebuggerInputHandler& InputHandler = Categories[CategoryId]->GetInputHandler(HandlerId);
			if (InputHandler.Mode == EDNADebuggerInputMode::Local || bHasAuthority)
			{
...
void ADNADebuggerCategoryReplicator::SendExtensionInputEvent(int32 ExtensionId, int32 HandlerId)
{
	if (HandlerId >= 0 && Extensions.IsValidIndex(ExtensionId) &&
		HandlerId < Extensions[ExtensionId]->GetNumInputHandlers())
	{
		FDNADebuggerInputHandler& InputHandler = Extensions[ExtensionId]->GetInputHandler(HandlerId);
		if (InputHandler.Mode == EDNADebuggerInputMode::Local || bHasAuthority)
		{
			InputHandler.Delegate.ExecuteIfBound();
		}
...
void ADNADebuggerCategoryReplicator::NotifyCategoriesToolState(bool bIsActive)
{
	for (int32 Idx = 0; Idx < Categories.Num(); Idx++)
	{
		FDNADebuggerCategory& CategoryOb = Categories[Idx].Get();
		if (bIsActive)
		{
			CategoryOb.OnDNADebuggerActivated();
		}
		else
		{
			CategoryOb.OnDNADebuggerDeactivated();
		}
	}
}

void ADNADebuggerCategoryReplicator::NotifyExtensionsToolState(bool bIsActive)
{
	for (int32 Idx = 0; Idx < Extensions.Num(); Idx++)
	{
		FDNADebuggerExtension& ExtensionOb = Extensions[Idx].Get();
		if (bIsActive)
		{
			ExtensionOb.OnDNADebuggerActivated();
		}
		else
		{
			ExtensionOb.OnDNADebuggerDeactivated();
		}
	}
}

bool ADNADebuggerCategoryReplicator::IsCategoryEnabled(int32 CategoryId) const
{

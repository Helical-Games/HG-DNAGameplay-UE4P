...
UDNADebuggerLocalController::UDNADebuggerLocalController(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	bSimulateMode = false;
	bNeedsCleanup = false;
...
void UDNADebuggerLocalController::Initialize(ADNADebuggerCategoryReplicator& Replicator, ADNADebuggerPlayerManager& Manager)
{
	CachedReplicator = &Replicator;
	CachedPlayerManager = &Manager;
	bSimulateMode = FDNADebuggerAddonBase::IsSimulateInEditor();

	UDebugDrawService::Register(bSimulateMode ? TEXT("DebugAI") : TEXT("Game"), FDebugDrawDelegate::CreateUObject(this, &UDNADebuggerLocalController::OnDebugDraw));
	if (bSimulateMode)
	{
		USelection::SelectionChangedEvent.AddUObject(this, &UDNADebuggerLocalController::OnSelectionChanged);
	}

	FDNADebuggerAddonManager& AddonManager = FDNADebuggerAddonManager::GetCurrent();
	AddonManager.OnCategoriesChanged.AddUObject(this, &UDNADebuggerLocalController::OnCategoriesChanged);
	OnCategoriesChanged();

	const UDNADebuggerConfig* SettingsCDO = UDNADebuggerConfig::StaticClass()->GetDefaultObject<UDNADebuggerConfig>();
...
void UDNADebuggerLocalController::Cleanup()
{
	if (bSimulateMode)
	{
...
#if WITH_EDITOR
		FDNADebuggerEdMode::SafeCloseMode();
#endif // WITH_EDITOR
	}
...
void UDNADebuggerLocalController::BeginDestroy()
{
	Super::BeginDestroy();
	if (bNeedsCleanup)
	{
...
void UDNADebuggerLocalController::OnDebugDraw(class UCanvas* Canvas, class APlayerController* PC)
{
	if (CachedReplicator && CachedReplicator->IsEnabled())
	{
		FDNADebuggerCanvasContext CanvasContext(Canvas, GEngine->GetSmallFont());
		...
		{
			TSharedRef<FDNADebuggerCategory> Category = CachedReplicator->GetCategory(Idx);
...
void UDNADebuggerLocalController::DrawHeader(FDNADebuggerCanvasContext& CanvasContext)
{
...
#if WITH_EDITOR
		GLevelEditorModeTools().ActivateMode(FDNADebuggerEdMode::EM_DNADebugger);
#endif // WITH_EDITOR
	}
	else
	{
	...
	if (NumExtensionRows)
	{
		...
		{
			TSharedRef<FDNADebuggerExtension> Extension = CachedReplicator->GetExtension(ExtensionIdx);
			...
			if (ExtensionDesc.Len())
			{
				...
				{
					ExtensionRowDesc += FDNADebuggerCanvasStrings::SeparatorSpace;
				}
	...
	for (int32 RowIdx = 0; RowIdx < NumRows; RowIdx++)
	{
		...
		for (int32 Idx = 0; Idx < 10; Idx++)
		{
			...
			{
				TSharedRef<FDNADebuggerCategory> Category0 = CachedReplicator->GetCategory(SlotCategoryIds[CategorySlotIdx][0]);
				const bool bIsEnabled = Category0->IsCategoryEnabled();
				const FString CategoryColorName = (RowIdx == ActiveRowIdx) && (NumRows > 1) ?
					(bIsEnabled ? *FDNADebuggerCanvasStrings::ColorNameEnabledActiveRow : *FDNADebuggerCanvasStrings::ColorNameDisabledActiveRow) :
					(bIsEnabled ? *FDNADebuggerCanvasStrings::ColorNameEnabled : *FDNADebuggerCanvasStrings::ColorNameDisabled);

				const FString CategoryDesc = (RowIdx == ActiveRowIdx) ?
					FString::Printf(TEXT("%s{%s}%d:{%s}%s"),
						Idx ? *FDNADebuggerCanvasStrings::SeparatorSpace : TEXT(""),
						*FDNADebuggerCanvasStrings::ColorNameInput,
						Idx,
						*CategoryColorName,
						*SlotNames[CategorySlotIdx]) :
					FString::Printf(TEXT("%s{%s}%s"),
						Idx ? *FDNADebuggerCanvasStrings::Separator : TEXT(""),
						*CategoryColorName,
						*SlotNames[CategorySlotIdx]);
...
void UDNADebuggerLocalController::DrawCategoryHeader(int32 CategoryId, TSharedRef<FDNADebuggerCategory> Category, FDNADebuggerCanvasContext& CanvasContext)
{
		...
		Category->GetNumDataPacks() > 0)
	{
		...
		{
			TSharedRef<FDNADebuggerCategory> MappedCategory = CachedReplicator->GetCategory(DataPackMap[CategoryId][Idx]);
			for (int32 DataPackIdx = 0; DataPackIdx < MappedCategory->GetNumDataPacks(); DataPackIdx++)
			{
				FDNADebuggerDataPack::FHeader DataHeader = MappedCategory->GetDataPackHeaderCopy(DataPackIdx);
...
void UDNADebuggerLocalController::BindInput(UInputComponent& InputComponent)
{
	...
	if (!bSimulateMode)
	{
		InputComponent.BindKey(SettingsCDO->ActivationKey, IE_Pressed, this, &UDNADebuggerLocalController::OnActivationPressed);
		InputComponent.BindKey(SettingsCDO->ActivationKey, IE_Released, this, &UDNADebuggerLocalController::OnActivationReleased);
		NewBindings.Add(SettingsCDO->ActivationKey.GetFName());
	}

	if (bIsLocallyEnabled || bSimulateMode)
	{
		InputComponent.BindKey(SettingsCDO->CategorySlot0, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory0Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot1, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory1Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot2, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory2Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot3, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory3Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot4, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory4Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot5, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory5Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot6, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory6Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot7, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory7Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot8, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory8Pressed);
		InputComponent.BindKey(SettingsCDO->CategorySlot9, IE_Pressed, this, &UDNADebuggerLocalController::OnCategory9Pressed);

		InputComponent.BindKey(SettingsCDO->CategoryRowPrevKey, IE_Pressed, this, &UDNADebuggerLocalController::OnCategoryRowUpPressed);
		InputComponent.BindKey(SettingsCDO->CategoryRowNextKey, IE_Pressed, this, &UDNADebuggerLocalController::OnCategoryRowDownPressed);
		...
		for (int32 Idx = 0; Idx < NumCategories; Idx++)
		{
			...
			{
				FDNADebuggerInputHandler& HandlerData = Category->GetInputHandler(HandlerIdx);
				if (HandlerData.Modifier.bPressed || HandlerData.Modifier.bReleased)
				{
					...
					InputBinding.KeyDelegate.GetDelegateForManualSet().BindUObject(this, &UDNADebuggerLocalController::OnCategoryBindingEvent, Idx, HandlerIdx);
		...
		{
			TSharedRef<FDNADebuggerExtension> Extension = CachedReplicator->GetExtension(Idx);
			const int32 NumInputHandlers = Extension->GetNumInputHandlers();

			for (int32 HandlerIdx = 0; HandlerIdx < NumInputHandlers; HandlerIdx++)
			{
				FDNADebuggerInputHandler& HandlerData = Extension->GetInputHandler(HandlerIdx);
				if (HandlerData.Modifier.bPressed || HandlerData.Modifier.bReleased)
				{
					FInputChord InputChord(FKey(HandlerData.KeyName), HandlerData.Modifier.bShift, HandlerData.Modifier.bCtrl, HandlerData.Modifier.bAlt, HandlerData.Modifier.bCmd);
					FInputKeyBinding InputBinding(InputChord, HandlerData.Modifier.bPressed ? IE_Pressed : IE_Released);
					InputBinding.KeyDelegate.GetDelegateForManualSet().BindUObject(this, &UDNADebuggerLocalController::OnExtensionBindingEvent, Idx, HandlerIdx);
...
bool UDNADebuggerLocalController::IsKeyBound(const FName KeyName) const
{
	return UsedBindings.Contains(KeyName);
}

void UDNADebuggerLocalController::OnActivationPressed()
{
	bPrevLocallyEnabled = bIsLocallyEnabled;
	if (CachedReplicator)
	{
		CachedReplicator->GetWorldTimerManager().SetTimer(StartSelectingActorHandle, this, &UDNADebuggerLocalController::OnStartSelectingActor, 0.2f);
	}
}

void UDNADebuggerLocalController::OnActivationReleased()
{
	if (CachedReplicator)
	{
...
void UDNADebuggerLocalController::OnCategory0Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 0);
}

void UDNADebuggerLocalController::OnCategory1Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 1);
}

void UDNADebuggerLocalController::OnCategory2Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 2);
}

void UDNADebuggerLocalController::OnCategory3Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 3);
}

void UDNADebuggerLocalController::OnCategory4Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 4);
}

void UDNADebuggerLocalController::OnCategory5Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 5);
}

void UDNADebuggerLocalController::OnCategory6Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 6);
}

void UDNADebuggerLocalController::OnCategory7Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 7);
}

void UDNADebuggerLocalController::OnCategory8Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 8);
}

void UDNADebuggerLocalController::OnCategory9Pressed()
{
	ToggleSlotState((ActiveRowIdx * 10) + 9);
}

void UDNADebuggerLocalController::OnCategoryRowUpPressed()
{
	const int32 NumRows = (NumCategorySlots + 9) / 10;
	ActiveRowIdx = (NumRows > 1) ? ((ActiveRowIdx + NumRows - 1) % NumRows) : 0;
}

void UDNADebuggerLocalController::OnCategoryRowDownPressed()
{
	const int32 NumRows = (NumCategorySlots + 9) / 10;
	ActiveRowIdx = (NumRows > 1) ? ((ActiveRowIdx + 1) % NumRows) : 0;
}

void UDNADebuggerLocalController::OnCategoryBindingEvent(int32 CategoryId, int32 HandlerId)
{
	if (CachedReplicator)
	{
...
void UDNADebuggerLocalController::OnExtensionBindingEvent(int32 ExtensionId, int32 HandlerId)
{
	if (CachedReplicator)
	{
...
void UDNADebuggerLocalController::OnStartSelectingActor()
{
	StartSelectingActorHandle.Invalidate();
	if (CachedReplicator)
	{
		...
		CachedReplicator->GetWorldTimerManager().SetTimer(SelectActorTickHandle, this, &UDNADebuggerLocalController::OnSelectActorTick, 0.01f, bLooping);

		OnSelectActorTick();
	}
}

void UDNADebuggerLocalController::OnSelectActorTick()
{
...
void UDNADebuggerLocalController::ToggleSlotState(int32 SlotIdx)
{
...
FString UDNADebuggerLocalController::GetKeyDescriptionShort(const FKey& KeyBind) const
{
	return FString::Printf(TEXT("[%s]"), *KeyBind.GetFName().ToString());
}

FString UDNADebuggerLocalController::GetKeyDescriptionLong(const FKey& KeyBind) const
{
...
void UDNADebuggerLocalController::OnSelectionChanged(UObject* Object)
{
...
void UDNADebuggerLocalController::OnCategoriesChanged()
{
	FDNADebuggerAddonManager& AddonManager = FDNADebuggerAddonManager::GetCurrent();
...
void UDNADebuggerLocalController::RebuildDataPackMap()
{
	...
	for (int32 SlotIdx = 0; SlotIdx < NumCategorySlots; SlotIdx++)
	{
		...
		for (int32 InnerIdx = 0; InnerIdx < SlotCategoryIds[SlotIdx].Num(); InnerIdx++)
		{
			...
			TSharedRef<FDNADebuggerCategory> Category = CachedReplicator->GetCategory(CategoryId);
			if (!Category->IsCategoryHeaderVisible())
			{

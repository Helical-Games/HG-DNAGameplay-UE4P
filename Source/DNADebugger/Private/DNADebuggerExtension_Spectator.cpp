...
FDNADebuggerExtension_Spectator::FDNADebuggerExtension_Spectator()
{
	const FDNADebuggerInputHandlerConfig KeyConfig(TEXT("Toggle"), EKeys::Tab.GetFName());
	bHasInputBinding = BindKeyPress(KeyConfig, this, &FDNADebuggerExtension_Spectator::ToggleSpectatorMode);
}

TSharedRef<FDNADebuggerExtension> FDNADebuggerExtension_Spectator::MakeInstance()
{
	return MakeShareable(new FDNADebuggerExtension_Spectator());
}

void FDNADebuggerExtension_Spectator::OnDeactivated()
{
...
FString FDNADebuggerExtension_Spectator::GetDescription() const
{
		...
		CachedDescription = !bHasInputBinding ? FString() :
			FString::Printf(TEXT("{%s}%s:{%s}Spectator"),
				...
				*GetInputHandlerDescription(0),
				SpectatorController.IsValid() ? *FDNADebuggerCanvasStrings::ColorNameEnabled : *FDNADebuggerCanvasStrings::ColorNameDisabled);
...
void FDNADebuggerExtension_Spectator::ToggleSpectatorMode()
{
	APlayerController* OwnerPC = GetPlayerController();
	ADNADebuggerPlayerManager& PlayerManager = ADNADebuggerPlayerManager::GetCurrent(OwnerPC->GetWorld());

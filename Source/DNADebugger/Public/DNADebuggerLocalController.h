...
#include "DNADebuggerLocalController.generated.h"

class AActor;
class ADNADebuggerCategoryReplicator;
class ADNADebuggerPlayerManager;
class FDNADebuggerCanvasContext;
class FDNADebuggerCategory;
class UInputComponent;
struct FKey;

UCLASS(NotBlueprintable, NotBlueprintType, noteditinlinenew, hidedropdown, Transient)
class UDNADebuggerLocalController : public UObject
{
	...
	void Initialize(ADNADebuggerCategoryReplicator& Replicator, ADNADebuggerPlayerManager& Manager);
...
protected:
	...
	UPROPERTY()
	ADNADebuggerCategoryReplicator* CachedReplicator;

	UPROPERTY()
	ADNADebuggerPlayerManager* CachedPlayerManager;
	...
	/** draw header row */
	void DrawHeader(FDNADebuggerCanvasContext& CanvasContext);

	/** draw header for category */
	void DrawCategoryHeader(int32 CategoryId, TSharedRef<FDNADebuggerCategory> Category, FDNADebuggerCanvasContext& CanvasContext);

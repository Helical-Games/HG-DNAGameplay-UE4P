...
#include "DNADebuggerConfig.generated.h"

struct FDNADebuggerInputModifier;

UENUM()
enum class EDNADebuggerOverrideMode : uint8
{
	Enable,
	Disable,
	UseDefault,
...
USTRUCT()
struct FDNADebuggerInputConfig
{
...
USTRUCT()
struct FDNADebuggerCategoryConfig
{
	GENERATED_USTRUCT_BODY()
	...
	EDNADebuggerOverrideMode ActiveInGame;

	UPROPERTY(EditAnywhere, Category = Settings)
	EDNADebuggerOverrideMode ActiveInSimulate;

	UPROPERTY(EditAnywhere, Category = Settings)
	EDNADebuggerOverrideMode Hidden;

	UPROPERTY(EditAnywhere, Category = Settings, AdvancedDisplay)
	uint32 bOverrideSlotIdx : 1;

	UPROPERTY(EditAnywhere, Category = Settings, EditFixedSize)
	TArray<FDNADebuggerInputConfig> InputHandlers;

	FDNADebuggerCategoryConfig() :
		ActiveInGame(EDNADebuggerOverrideMode::UseDefault), ActiveInSimulate(EDNADebuggerOverrideMode::UseDefault),
		Hidden(EDNADebuggerOverrideMode::UseDefault), bOverrideSlotIdx(false)
	{}
};

USTRUCT()
struct FDNADebuggerExtensionConfig
{
	GENERATED_USTRUCT_BODY()
	...
	UPROPERTY(EditAnywhere, Category = Settings)
	EDNADebuggerOverrideMode UseExtension;

	UPROPERTY(EditAnywhere, Category = Settings, EditFixedSize)
	TArray<FDNADebuggerInputConfig> InputHandlers;

	FDNADebuggerExtensionConfig() : UseExtension(EDNADebuggerOverrideMode::UseDefault) {}
};

UCLASS(config = Engine, defaultconfig)
class DNADEBUGGER_API UDNADebuggerConfig : public UObject
{
	...
	TArray<FDNADebuggerCategoryConfig> Categories;

	UPROPERTY(config, EditAnywhere, Category = AddOns, EditFixedSize)
	TArray<FDNADebuggerExtensionConfig> Extensions;
	...
	void UpdateCategoryInputConfig(const FName CategoryName, const FName InputName, FName& KeyName, FDNADebuggerInputModifier& KeyModifier);
	...
	/** updates entry in Categories array and modifies input binding params */
	void UpdateExtensionInputConfig(const FName ExtensionName, const FName InputName, FName& KeyName, FDNADebuggerInputModifier& KeyModifier);

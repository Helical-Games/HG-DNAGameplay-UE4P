...
// DNA DEBUGGER CATEGORY
// 
// Single category of DNA debugger tool, responsible for gathering and presenting data.
...
// 
// 
// Categories needs to be manually registered and unregistered with DNADebugger.
 ...
 */
class DNADEBUGGER_API FDNADebuggerCategory : public FDNADebuggerAddonBase
{
public:

	FDNADebuggerCategory();
	virtual ~FDNADebuggerCategory();
	...
	virtual void DrawData(APlayerController* OwnerPC, FDNADebuggerCanvasContext& CanvasContext);
	...
	void AddShape(const FDNADebuggerShape& Shape);

	/** [LOCAL] draw category */
	void DrawCategory(APlayerController* OwnerPC, FDNADebuggerCanvasContext& CanvasContext);
	...
	FDNADebuggerDataPack::FHeader GetDataPackHeaderCopy(int32 DataPackId) const { return ReplicatedDataPacks.IsValidIndex(DataPackId) ? ReplicatedDataPacks[DataPackId].Header : FDNADebuggerDataPack::FHeader(); }
	...
	TArray<FDNADebuggerShape> GetReplicatedShapesCopy() const { return ReplicatedShapes; }
...
protected:
	 ...
	 */
	template<typename T>
	int32 SetDataPackReplication(T* DataPackAddr, EDNADebuggerDataPack Flags = EDNADebuggerDataPack::ResetOnTick)
	{
		FDNADebuggerDataPack NewDataPack;
...
private:
	...
	friend class ADNADebuggerCategoryReplicator;
	friend struct FDNADebuggerNetPack;
	...
	TArray<FDNADebuggerShape> ReplicatedShapes;

	/** list of replicated data packs */
	TArray<FDNADebuggerDataPack> ReplicatedDataPacks;
};

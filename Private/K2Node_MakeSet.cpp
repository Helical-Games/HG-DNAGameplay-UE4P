...
static const FString SetOutputPinName = FString(TEXT("Set"));

#define LOCTEXT_NAMESPACE "MakeSetNode"

/////////////////////////////////////////////////////
// FKCHandler_MakeSet
class FKCHandler_MakeSet : public FNodeHandlingFunctor
{
public:
	...
	{
 		UK2Node_MakeSet* SetNode = CastChecked<UK2Node_MakeSet>(Node);
 		UEdGraphPin* OutputPin = SetNode->GetOutputPin();

		FNodeHandlingFunctor::RegisterNets(Context, Node);

		// Create a local term to drop the set into
	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		...
		FBPTerminal** SetTerm = Context.NetMap.Find(OutputPin);
		check(SetTerm);

		FBlueprintCompiledStatement& CreateSetStatement = Context.AppendStatementForNode(Node);
		CreateSetStatement.Type = KCST_CreateArray;
		CreateSetStatement.LHS = *SetTerm;
		...
		for(auto PinIt = Node->Pins.CreateIterator(); PinIt; ++PinIt)
		{
			...
			if(Pin && Pin->Direction == EGPD_Input)
			{
				...
				if( InputTerm )
				{
					CreateSetStatement.RHS.Add(*InputTerm);
				}
			}
		}
	}
...
UEdGraphPin* UK2Node_MakeSet::GetOutputPin() const
{
	return FindPin(SetOutputPinName);
}

void UK2Node_MakeSet::ReallocatePinsDuringReconstruction(TArray<UEdGraphPin*>& OldPins)
{
...
void UK2Node_MakeSet::AllocateDefaultPins()
{
	// Create the output pin
	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Wildcard, TEXT(""), NULL, false, false, *FString::Printf(TEXT("[%d]")));

	// Create the input pins to create the set from
...
bool UK2Node_MakeSet::CanResetToWildcard() const
{
...
void UK2Node_MakeSet::ClearPinTypeToWildcard()
{
...
void UK2Node_MakeSet::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
void UK2Node_MakeSet::PropagatePinType()
{
...
void UK2Node_MakeSet::PostReconstructNode()
{
...
{
	// @todo: replace MakeArray_16x icon with MakeSet_16x icon (when that icon is available/created)
	static FSlateIcon Icon("EditorStyle", "GraphEditor.MakeArray_16x");
	return Icon;
}

void UK2Node_MakeSet::AddInputPin()
{
...
void UK2Node_MakeSet::SyncPinNames()
{
...
void UK2Node_MakeSet::RemoveInputPin(UEdGraphPin* Pin)
{
...
void UK2Node_MakeSet::GetContextMenuActions(const FGraphNodeContextMenuBuilder& Context) const
{
	...
	if (!Context.bIsDebugging)
	{
		...
		if (Context.Pin != NULL)
		{
			...
			{
				//@TODO: Only offer this option on pins
...
bool UK2Node_MakeSet::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
	...
	{
		OutReason = NSLOCTEXT("K2Node", "MakeSet_InputIsContainer", "Cannot make a set with an input of a container!").ToString();
		return true;
	}
	...
	{
		OutReason = NSLOCTEXT("K2Node", "MakeSet_InputIsExec", "Cannot make a set with an execution input!").ToString();
		return true;
	}
...
void UK2Node_MakeSet::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
